...

{
  ...
  "additional_pu_stops": "if the RC lists MORE THAN ONE pickup location (multi-stop load), an array of the 2nd, 3rd, etc. pickups IN ORDER, each as {\"address\": same multi-line format as pu_address, \"date\": ..., \"time\": ... (same HH:MM rules as pu_time), \"reference\": PU#/reference for that specific stop if shown, else null}. Empty array if there's only one pickup.",
  ...
  "additional_del_stops": "same idea as additional_pu_stops, but for extra delivery/drop-off stops beyond the first - array of {\"address\": ..., \"date\": ..., \"time\": ..., \"reference\": ...}. Empty array if there's only one delivery.",
